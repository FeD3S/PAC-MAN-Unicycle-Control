...
res_global = struct();
dt = 0.001;
T_track = 25; % Duration of the parking trajectory
...
% Using your provided transformation logic:
dx = q_d_posture(1) - xf;
dy = q_d_posture(2) - yf;
...
% Calculate Posture Error Manually (Error = Desired - Actual)
% We wrap the theta error to [-pi, pi]
...
% Shift time of second simulation by the end time of the first
t2_shifted = t2 + t1(end);

res_global.time  = [t1; t2_shifted];
res_global.pose  = [p1; p2];
res_global.error = [e1; e2];
res_global.p_ref = [r1; r2];

% Save metadata (optional but helpful for plotting)
res_global.switch_time = t1(end);
res_global.gains_track = [KNL_b, KNL_xi];
res_global.gains_posture = K_posture;

% --- 6. SAVE ---
saveDir = fullfile('Results','full_tasks');
if ~exist(saveDir, 'dir'), mkdir(saveDir); end

% Define your filename (using dummy suffix if variable not defined)
if ~exist('folder_suffix', 'var'), folder_suffix = 'combined_sim'; end
fileName = fullfile(saveDir, [folder_suffix, '.mat']);
save(fileName, '-struct', 'res_global');
